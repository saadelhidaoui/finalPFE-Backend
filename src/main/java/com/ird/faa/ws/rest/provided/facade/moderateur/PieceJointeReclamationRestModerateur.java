...
@RestController
@RequestMapping("api/moderateur/pieceJointeReclamation")
public class PieceJointeReclamationRestModerateur {

    @Autowired
    private PieceJointeReclamationModerateurService pieceJointeReclamationService;
    ...
    @PutMapping("/upload/reclamation/{reference}")
    public void uploadFile(@RequestParam("file") MultipartFile file, @PathVariable String reference) throws IOException {
        pieceJointeReclamationService.uploadFile(file,reference);

    }
    @GetMapping("/files/{id}")
    public ResponseEntity<byte[]> getFile(@PathVariable Long id) {
        PieceJointeReclamation pieceJointeReclamation = pieceJointeReclamationService.getFile(id);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + pieceJointeReclamation.getLibelle() + "\"")
                .body(pieceJointeReclamation.getData());
    }
}
