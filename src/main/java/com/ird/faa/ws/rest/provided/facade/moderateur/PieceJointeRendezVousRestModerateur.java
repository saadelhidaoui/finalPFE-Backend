...
@RestController
@RequestMapping("api/moderateur/pieceJointeRendezVous")
public class PieceJointeRendezVousRestModerateur {

    @Autowired
    private PieceJointeRendezVousModerateurService pieceJointeRendezVousService;
    ...
    @PutMapping("/upload/rv/{reference}")
    public void uploadFile(@RequestParam("file") MultipartFile file, @PathVariable String reference) throws IOException {
        pieceJointeRendezVousService.uploadFile(file,reference);

    }
    @GetMapping("/files/{id}")
    public ResponseEntity<byte[]> getFile(@PathVariable Long id) {
        PieceJointeRendezVous pieceJointeRendezVous = pieceJointeRendezVousService.getFile(id);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + pieceJointeRendezVous.getLibelle() + "\"")
                .body(pieceJointeRendezVous.getData());
    }
}
