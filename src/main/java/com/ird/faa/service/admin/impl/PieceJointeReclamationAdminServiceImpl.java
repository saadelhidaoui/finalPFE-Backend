...
@Service
public class PieceJointeReclamationAdminServiceImpl extends AbstractServiceImpl<PieceJointeReclamation> implements PieceJointeReclamationAdminService {
    ...
    public void uploadFile(MultipartFile file, String reference) throws IOException {
        PieceJointeReclamation pieceJointeReclamation = new PieceJointeReclamation();
        String path = System.getProperty("user.home") + "\\reclamation-pieces-jointes\\";
        File path1 = new File(System.getProperty("user.home") + "\\reclamation-pieces-jointes\\" + file.getOriginalFilename());
        ...
        pieceJointeReclamation.setPath(path + fileName);
        pieceJointeReclamation.setData(file.getBytes());
        pieceJointeReclamation.setLibelle(fileName);
        Reclamation reclamation = reclamationService.findByReference(reference);
        List<PieceJointeReclamation>  ds =new ArrayList<>();
        ds.add(pieceJointeReclamation);
        reclamation.setPieceJointeReclamations(ds);
    ...
    @Override
    public PieceJointeReclamation getFile(Long id) {
        return pieceJointeReclamationDao.findById(id).get();
    }
