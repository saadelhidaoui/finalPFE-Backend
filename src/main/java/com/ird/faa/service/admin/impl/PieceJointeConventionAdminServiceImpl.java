...
public class PieceJointeConventionAdminServiceImpl extends AbstractServiceImpl<PieceJointeConvention> implements PieceJointeConventionAdminService {
    ...
    public void uploadFile(MultipartFile file, String reference) throws IOException {
        PieceJointeConvention pieceJointeConvention = new PieceJointeConvention();
        String path = System.getProperty("user.home") + "\\convention-pieces-jointes\\";
        File path1 = new File(System.getProperty("user.home") + "\\convention-pieces-jointes\\" + file.getOriginalFilename());
        ...
        pieceJointeConvention.setPath(path + fileName);
        pieceJointeConvention.setData(file.getBytes());
        pieceJointeConvention.setLibelle(fileName);
//        demandePieceJointe.setType(file.getContentType());
        Convention convention = conventionService.findByReference(reference);
        List<PieceJointeConvention>  ds =new ArrayList<>();
        ds.add(pieceJointeConvention);
        convention.setPieceJointeConventions(ds);
...
//
    @Override
    public PieceJointeConvention getFile(Long id) {
        return pieceJointeConventionDao.findById(id).get();
    }
